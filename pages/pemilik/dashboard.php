...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['aksi'] ?? '') === 'tambah_user') {
    ...
    if ($nama === '' || $username === '' || $password === '' || !in_array($role, $roles_valid, true)) {
        ...
        if ($cek->num_rows > 0) {
            ...
            $ins = $db->prepare(
                "INSERT INTO users (nama, username, password, role, aktif, dibuat_oleh) VALUES (?,?,?,?,1,?)"
            );
...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-primary {
            width: 100%; padding: .75rem; background: linear-gradient(135deg, var(--accent), var(--accent2));
        ...
        /* Toggle button */
        .btn-toggle {
            padding: .3rem .75rem; border-radius: 6px; font-size: .78rem; cursor: pointer;
